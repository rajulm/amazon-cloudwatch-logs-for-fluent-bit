...
func (output *OutputPlugin) AddEvent(tag string, record map[interface{}]interface{}, timestamp time.Time) int {
	...
	logrus.Infof("[rajulm %d] Appending event %s and timestamp %s, in millis as %d\n", tag, event, timestamp, aws.Int64(timestamp.UnixNano() / 1e6))
...
func (output *OutputPlugin) putLogEvents(stream *logStream) error {
	...
	logrus.Infof("[rajulm kajeeth] Log events before sorting are %s\n", stream.logEvents)
	...
	logrus.Infof("[rajulm kajeeth] Log events after sorting are %s\n", stream.logEvents)
